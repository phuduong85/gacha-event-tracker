...
describe("NextUp", () => {
  ...
  test("leads with the closest deadline and lists the ones behind it", () => {
    ...
    const html = render(<NextUp rows={rows} focused={null} onOpen={() => {}} collapsed={false} onToggleCollapsed={() => {}} meterMode="percentage" />);
  ...
  test("one deadline is a headline with nothing behind it", () => {
    const html = render(
      <NextUp rows={rows.slice(0, 1)} focused={null} onOpen={() => {}} collapsed={false} onToggleCollapsed={() => {}} meterMode="percentage" />,
    );
    expect(html).toContain("Closing Ceremony");
  ...
  test("no deadlines says so rather than rendering an empty panel", () => {
    const html = render(<NextUp rows={[]} focused={null} onOpen={() => {}} collapsed={false} onToggleCollapsed={() => {}} meterMode="percentage" />);
  ...
  test("an unannounced end is never dressed up as a countdown", () => {
    ...
    const html = render(
      <NextUp rows={[row("Unknown End", "wuwa", null)]} focused={null} onOpen={() => {}} collapsed={false} onToggleCollapsed={() => {}} meterMode="percentage" />,
    );
